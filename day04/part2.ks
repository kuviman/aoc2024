...
let solve = fn(path) {
    ...
    let find_single = fn(pos :: vec2, dir :: vec2) -> bool {
        let neg_dir = vec2_neg(dir);
        find_at(vec2_add(pos, dir), vec2_neg(dir))
        or
        find_at(vec2_add(pos, neg_dir), vec2_neg(neg_dir))
    };

    let mut answer :: int32 = 0;
    for x :: int32 in range(1, width - 1) {
        dbg (x, width);
        for y :: int32 in range(1, height - 1) {
            if find_single((x, y), (1, 1)) and find_single((x, y), (1, -1)) then (
                answer += 1;
            );
        }
    };
