...
let solve = fn(path) {
    ...
    let add_antinode = fn(pos :: Pos) -> bool {
        if 0 <= pos.x and pos.x < input.size.x and 0 <= pos.y and pos.y < input.size.y then (
            if HashMap_get[_, _] (unique_antinodes, pos) is :Some _ then false else (
    ...
    for c :: char, positions :: list[Pos] in HashMap_iter[_, _] input.antennas {
        ...
        for i in range(0, list_length positions) {
            ...
            for j in range(0, i) {
                ...
                # dbg (.a, .b);
                # dbg (.dx, .dy);
                for steps in range(-50, 50) {
                    let pos = (.x = a.x + dx * steps, .y = a.y + dy * steps);
                    if add_antinode pos then (
                        # dbg pos;
                    );
                }
            }
