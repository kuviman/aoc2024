...
let solve = fn(path) {
    ...
    let cmp = (a, b) => (
        unwindable cmp (
            if a == b then (unwind cmp 0);
            if binary_search (rules, (a, b)) then (unwind cmp (-1));
            if binary_search (rules, (b, a)) then (unwind cmp (+1));
            panic "wtf badcop lied"; 0
        )
    );

    let mut answer = 0;
    let mut index :: int32 = 0;
    for mut update :: list[int32] in list_iter updates {
        dbg index;
        index += 1;
        ...
        if not is_correct then (
            update = sort_by (update, cmp);
            /*dbg update;
            for i :: int32 in range(1, n) {
                if cmp(list_get(update, i - 1), list_get(update, i)) >= 0 then (
                    panic "not sorted huh";
                );
            };*/
